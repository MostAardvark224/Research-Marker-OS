...
MCP_LAUNCHER_FILENAME = "run-mcp"
# Relative to the AppImage mount root (electron-builder extraResources → resources/backend).
...
def mcp_launcher_path() -> Path:
    return Path(get_app_data_dir()) / MCP_LAUNCHER_FILENAME


def _is_ephemeral_executable(path: Path) -> bool:
    """AppImage mounts live under /tmp/.mount_* and vanish when the app quits."""
    text = str(path)
    try:
        text = str(path.resolve())
    except OSError:
        pass
    return "/tmp/.mount_" in text
...
def _write_appimage_mcp_launcher(appimage: Path) -> Path:
    """Stable Claude Desktop entrypoint that remounts the AppImage, then runs `api mcp`."""
...
set -euo pipefail
APPIMAGE={shlex.quote(str(appimage))}
BACKEND_REL={shlex.quote(APPIMAGE_BACKEND_REL)}
if [[ ! -f "$APPIMAGE" ]]; then
...
def ensure_stable_mcp_launcher() -> Path | None:
    """Install a durable MCP command when the frozen binary path is ephemeral.

    Claude Desktop keeps mcpServers.command across restarts. AppImage mount paths
    like /tmp/.mount_*/resources/backend/api disappear when Research Marker quits,
    which surfaces as spawn ENOENT. Point Claude at ~/.config/.../run-mcp instead.
    """
    if not getattr(sys, "frozen", False):
        return None
    exe = Path(sys.executable)
    if not _is_ephemeral_executable(exe):
        return None
    appimage = _appimage_path()
    if appimage is None:
        return None
    return _write_appimage_mcp_launcher(appimage)
...
def resolve_command_and_args() -> tuple[str, list[str], dict[str, str]]:
    ...
    if getattr(sys, "frozen", False):
        launcher = ensure_stable_mcp_launcher()
        if launcher is not None and launcher.is_file():
            return str(launcher), [], env
        if _is_ephemeral_executable(Path(sys.executable)):
            existing = mcp_launcher_path()
            if existing.is_file() and os.access(existing, os.X_OK):
                return str(existing), [], env
