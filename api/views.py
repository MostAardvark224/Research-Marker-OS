...
- NOTE: Should highlights be included as well? If there are many highlights than it could become bloated
- Will try to go with a relatively low dim count, fp32
- Gemini API might also be a good idea since low dim embeddings should be cheap 

2. Expirement a little bit to find the best cos similarity (should be easy to change later) between embeddings.
- Starting with arbitrary value of 0.9, will adjust once I have more data

3. Create clusters of papers based on cos similarity 
- I do want the clusters to be relatively large because there will be sub-topics as well 

4. Send clusters off to LLM, ask for a singular major topic + sub-topics 
- Use tool call to return JSON for parsing 
- Use Model to assign papers to subtopics
- Run checks to make sure that each paper is assigned and the model doesn't have any extraneous behavior, i.e. assigns papers that don't exist

5. Embed the topics and subtopics that are returned
- NOTE: find a good way to store all embeddings 

6. Use UMAP to project each vector embedding down to 2D (so that the vector can be represented on a graph)
- Find some way to make sure that there's no overlapping between layers 
    - ex: (12, 3) for subtopic as well as major topic

7. Save all data to backend 
- embeddings 
- model outputs 
- 2d coordinates attached to notes 
